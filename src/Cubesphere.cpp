...
Cubesphere::Cubesphere(float radius, int sub, bool smooth) : radius(radius), subdivision(sub), smooth(smooth), interleavedStride(32)
{
    ...
    noiseOutput.reserve(vertexCountPerFace);
...
//std::vector <float> Cubesphere::makeNoise() {
//    int vSize = vertexCountPerFace * 6;
//    int maxsize = cbrt(vSize);//sqrt(vertexCountPerFace)
...
//    std::vector <float> noiseO(vSize);
//    // Generate a 16 x 16 x 16 area of noise
//    //fnGenerator->GenUniformGrid3D(noiseO.data(), 0, 0, 0, maxsize, maxsize, maxsize, 1.f/100.f, 1337);
//    fnGenerator->GenUniformGrid3D(noiseO.data(), 0, 0, 0, maxsize, maxsize, maxsize, 1.f / 100.f, 1337);
//    return noiseO;
//}
...
void Cubesphere::buildVerticesSmooth()
{
    ...
    for(unsigned int i = 0; i < vertexCountPerRow; ++i)
    {
        ...
        for(unsigned int j = 0; j < vertexCountPerRow; ++j, k += 3, ++k1, ++k2)
        {
            ...
            //std::cout << noiseOutput[noiseIndex] << std::endl;
    ...
    startIndex = vertices.size() / 3;
    //noiseOutput = makeNoise();

    for(int i = 0, j = 0; i < vertexSize; i += 3, j += 2)
    {
        addVertex(-vertices[i] * (noiseOutput[noiseIndex] + radius), vertices[i + 1], -vertices[i + 2]);
        addTexCoord(texCoords[j], texCoords[j+1]);
        addNormal(-normals[i], normals[i+1], -normals[i+2]);
        std::cout << noiseIndex << " : " << noiseOutput[noiseIndex] + radius << std::endl;
        noiseIndex = noiseIndex + 3;
        //noiseIndex++;
    }
    for(int i = 0; i < indexSize; ++i)
    {
